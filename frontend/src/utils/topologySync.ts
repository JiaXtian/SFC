...
export function buildRuntimeTopologyPayload(
  ...
  return {
    ...
    nodes: satellites.map((s: any) => ({
      ...
      core_network_load: asFinite(s?.core_network_load, 0.5),
      core_business_load: {
        signaling_load: asFinite(s?.core_business_load?.signaling_load, asFinite(s?.core_network_load, 0.5)),
        session_load: asFinite(s?.core_business_load?.session_load, asFinite(s?.core_network_load, 0.5)),
        user_plane_load: asFinite(s?.core_business_load?.user_plane_load, asFinite(s?.core_network_load, 0.5)),
        mobility_load: asFinite(s?.core_business_load?.mobility_load, asFinite(s?.core_network_load, 0.5)),
        policy_load: asFinite(s?.core_business_load?.policy_load, asFinite(s?.core_network_load, 0.5)),
        auth_load: asFinite(s?.core_business_load?.auth_load, asFinite(s?.core_network_load, 0.5)),
      },
