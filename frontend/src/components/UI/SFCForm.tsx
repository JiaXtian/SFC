...
export default function SFCForm() {
  const { setCandidateResult, addToast, topologyVersion, backendTopologySynced, satellites } = useStore()
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (sortedCandidates.length === 0) {
        const errorMsg = result.error || 'No feasible deployment found'
      ...
      const feasible = sortedCandidates.filter((c: any) => c.satisfies_constraints)
      ...
      if (feasible.length === 0) {
        addToast(`生成 ${sortedCandidates.length} 个候选方案（均不满足SLA，可人工选择）`, 'warning')
      } else {
        addToast(`生成 ${sortedCandidates.length} 个候选方案`, 'success')
      }
      if (result.warning || sortedCandidates.length < (sfc.topk || 1)) {
        alert(result.warning || `仅生成 ${sortedCandidates.length} 个可行方案，少于请求的 Top-${sfc.topk || 1}。`)
      }
    } catch (err: any) {
      console.error('[SFCForm] Submit error:', err)
      const errorMsg = err.response?.data?.details || err.message || '未知错误'
